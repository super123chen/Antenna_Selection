...
label_df = pd.read_csv(r'/home/wwj/chenqiliang/8833and8844shuju/8833JTRAS_capacity_label_10.csv').iloc[:, 1]
...
model.to(device)
ResNet_Pre1 = model(torch.from_numpy(xTest[0:19994]).to(device))


########################################################333wo xiede
testStart=time()
pre_array1 = np.zeros((19994, n_class))
index1=torch.argmax(ResNet_Pre1,dim=1)
for i in range(19994):
...
xTest_np = np.array(xTest[0:19994])
...
for i in range(n_sample):
    label_array[i, label[i] - 1] = 1
###################
yTest_indices = np.array(yTest[:19994])
