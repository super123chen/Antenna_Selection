...
from uitls_8833_gain import computation_time
import math
...
label_df = pd.read_csv(r'/home/wwj/chenqiliang/8833and8844shuju/8833JTRAS_gain_label_10.csv').iloc[:, 1]
...
class MyNetwork(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(256, 3136)
...
ResNet_Pre1 = model(torch.from_numpy(xTest[0:19995]).to(device))


########################################################333wo xiede
testStart=time()
pre_array1 = np.zeros((19995, n_class))
...
xTest_np = np.array(xTest[0:19995])
...
for i in range(n_sample):
    label_array[i, label[i] - 1] = 1
###################
yTest_indices = np.array(yTest[:19995])
...
for i in range(19995):
    ...
    Pre_fullGian = math.sqrt(1 / 3) * np.linalg.norm(ArrayA, ord='fro')
    Pre_subGian = math.sqrt(1 / 3) * np.linalg.norm(Pre_sub, ord='fro')
